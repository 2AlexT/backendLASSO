...
const getSingleGestion = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .query(
        `Select * from asfi_gestion where identificador=@id_gestion and id_empresa=@id_empresa and indicador='A'`
      );
    if (!result.recordset[0]) {
      res.json({ Message: "No existe gestiones con esa empresa" });
    } else {
...
const getGestiones = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .query(
        `Select * from asfi_gestion where id_empresa=@id_empresa and indicador='A'`
      );
    if (!result.recordset[0]) {
      res.json({ Message: "No existe gestiones con esa empresa" });
    } else {
...
const createNewGestion = async (req, res) => {
  try {
    ...
    const pool = await getConnection();
    const resultNombreEmpresa = await pool
      .request()
      .input("id_empresa", sql.VarChar, id_empresa)
      .query(
    ...
    if (!resultNombreEmpresa.recordset[0]) {
      ...
      const resultGestionExiste = await pool
        ...
        .input("id_empresa", sql.VarChar, id_empresa)
        .query(
          `select * from asfi_gestion where gestion=@gestion and indicador='A' and id_empresa=@id_empresa`
        );
      if (!resultGestionExiste.recordset[0]) {
        await pool
          .request()
          .input("I_proceso", 0)
          .input("I_identificador", id_empresa)
          .input("I_id_empresa", id_empresa)
          .input("I_gestion", gestion)
          .input("I_usuario", decoded.id.identificador)
          .input("I_origen", 0)
          .output("O_msg_error", sql.VarChar)
          .execute("segabm_gestion");
        //CREAR SECCIONES COMPLETAS CON ARTICULOS
...
const modificarGestion = async (req, res, next) => {
  try {
    ...
    if (typeof id_gestion !== "number" || typeof gestion !== "number") {
      return res
        .status(400)
        .json({ msg: "Bad request. Solo se aceptan numeros" });
    }
    ...
    const resultNombreEmpresa = await pool
      ...
      .input("id_empresa", sql.VarChar, id_empresa)
      .query(
        `select identificador from asfi_empresa where identificador=@id_empresa and indicador='A'`
      );
    ...
    if (!resultNombreEmpresa.recordset[0]) {
      ...
      const resultGestionExiste = await pool
        ...
        .input("id_empresa", sql.VarChar, id_empresa)
        .query(
          `select * from asfi_gestion where identificador=@id_gestion and indicador='A' and id_empresa=@id_empresa`
        );
      if (!resultGestionExiste.recordset[0]) {
        res.json({ message: "Empresa con gestion no existe" });
      } else {
        const resultNewGestionExiste = await pool
          .request()
          .input("gestion", sql.VarChar, gestion)
          .input("id_empresa", sql.VarChar, id_empresa)
          .query(
        ...
        if (!resultNewGestionExiste.recordset[0]) {
          await pool
            ...
            .execute("segabm_gestion");
          res.json({
            message: `Se modifico la gestion con identificador :'${id_gestion}' a la siguiente ${gestion}`,
          });
        } else {
          res.json({
            message: "Valor a modificar ya existe dentro de la empresa",
          });
        }
      }
    }
...
const gestionDarAlta = async (req, res, next) => {
  try {
    ...
    if (typeof id_gestion !== "number") {
      return res
        .status(400)
        .json({ msg: "Valor numerico no dado para eliminar" });
    }
    token = req.headers.authorization.split(" ")[1];
    const decoded = await promisify(jwt.verify)(
      token,
      ...
      if (!resultGestionExiste.recordset[0]) {
        ...
        await pool
          ...
          .input(
            "I_gestion",
            sql.Int,
            resultGestionExiste.recordset[0]["gestion"]
          )
          .input("I_usuario", sql.Int, decoded.id.identificador)
          .input("I_origen", sql.Int, 0)
          .output("O_msg_error", sql.VarChar)
          .execute("segabm_gestion");
        res.json({
          message: `Se dio de alta la gestion :${resultGestionExiste.recordset[0]["gestion"]} de la empresa`,
        });
        pool.close();
      }
    }
