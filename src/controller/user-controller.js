...
const verificarRefreshToken = async (req, res, next) => {
  ...
  if (req.body.refreshToken != undefined && req.body.refreshToken != null) {
    ...
    return res.status(401).json({ Message: "valor invalido" });
  }
  const seEncuentraDentrodetokens = refreshToken.includes(
    refreshTokenVerificar
  );
  if (seEncuentraDentrodetokens == false) {
    return res
      .status(401)
      .json({ Message: "REFRESH TOKEN NO ESTA DENTRO DE LA LISTA" });
  } else {
    try {
...
const getUserToken = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .query(
        `select nombre,identificador from asfi_usuario where nombre=@nombres and indicador='A'`
      );
    if (!result.recordset[0]) {
      res.status(401).json({
        message: "No se encuentra en la base de datos registrese por favor",
      });
    } else {
...
const createNewUser = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .query(
        `select identificador from asfi_usuario where nombre=@nombreUsuario and indicador='A'`
      );
    if (!result.recordset[0]) {
      await pool
        .request()
      ...
      res.status(400).json({ message: "Ya existe el Usuario dentro de la DB" });
    }
    pool.close();
  } catch (err) {
    logger.error(err);
...
const modificarUser = async (req, res) => {
  try {
    ...
    if (
      username.match(/^ *$/) !== null ||
      newUsername.match(/^ *$/) !== null ||
      username == newUsername
    ) {
      //Verificar cuando tenga espacios igual preguntar**
      return res.status(400).json({
        msg: "Bad request. Pleas fill all fields or similar fields where requested",
      });
    }
...
const buscarId = async (identificador) => {
  try {
    ...
    const result = await pool
      ...
      .query(
        `select identificador from asfi_usuario where identificador=@identificador and indicador='A'`
      );
