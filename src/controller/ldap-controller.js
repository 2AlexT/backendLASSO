...
exports.login = async (req, res, next) => {
  ...
  try {
    ...
    req.body = { nombre: newNombre, username: nombre };
    next();
    await this.close();
  } catch (err) {
    logger.error("Error en el login AD incorrecto");
    return next(
      new AppError("No se encuentra dentro del dominio :" + err, 401)
    );
  }
};
...
function buscar(nombre) {
  return new Promise(function (resolve, reject) {
    client.search(
      "DC=corp,DC=fassil,DC=com,DC=bo",
      {
      ...
      (err, res) => {
        if (err) {
          return new AppError(
            "Error en unirse al search ldap :=>" + err.message,
            400
          );
        } else {
          //-----------
          ...
          res.on("error", (err) => {
            if (err) {
              return new AppError(
                "Error en la busqueda :=>" + err.message,
                400
              );
            }
          });
...
exports.protect = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError(
        "Token expirado o incorrecto, no puede pasar el protect",
        401
      )
    );
  }
